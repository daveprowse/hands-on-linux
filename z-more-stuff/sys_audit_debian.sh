...
if [ "$EUID" -ne 0 ]; then
  echo "⚠️ Warning: Some commands (like apt-get, dpkg, lshw, journalctl) require root privileges."
  echo "It is highly recommended to run this script with sudo: sudo ./sys_audit.sh"
  echo "------------------------------------------------------------------------"
fi
...
echo "# System Analytical Report" > "$OUTPUT_FILE"
...
append_section "APT Package Check" "apt-get check"
append_section "DPKG Audit" "dpkg --audit"

# --- Hardware & CPU ---
append_section "Block Devices" "lsblk"
append_section "CPU Architecture" "lscpu"
append_section "Hardware Summary" "lshw -short"
