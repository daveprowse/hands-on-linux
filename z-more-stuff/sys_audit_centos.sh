...
if [ "$EUID" -ne 0 ]; then
  echo "⚠️ Warning: Some commands (like dnf, rpm, lshw, journalctl) require root privileges."
  echo "It is highly recommended to run this script with sudo: sudo ./sys_audit_centos.sh"
  echo "------------------------------------------------------------------------"
fi
...
echo "# System Analytical Report (CentOS/RHEL)" > "$OUTPUT_FILE"
...
append_section "Listening Ports" "ss -tulpn" # Modified for CentOS process exposure
append_section "Internet Connectivity Test" "ping -c 3 1.1.1.1"

# --- Services & Packages (CentOS Modifications Here) ---
...
append_section "DNF Package Check" "dnf check" # Changed from apt-get check
append_section "RPM Package Audit" "rpm -Va --nofiles" # Changed from dpkg --audit
